...
class DecisionTree(object):
    ...
    def splitData(self, data, axis, value):
        """划分数据集，三个参数为待划分的数据集，划分数据集的特征，特征的返回值"""
        retData = []
        for feature in data:
    ...
    def majorityCnt(self, labelsList):
        ...
        sortedLabelsCount = sorted(
            labelsCount.items(), key=lambda k: k[1], reverse=True
        )  # 排序，True升序
        # 返回出现次数最多的
    ...
    def createTree(self, data, features):
        ...
        # 清空 features[bestFeat]，在下一次使用时清零
    ...
    def predict(self, tree, features, x):
        for key1 in tree.keys():
            ...
            # key是根节点的代表特征，featIndex是取根节点特征在特征列表中的索引，方便后续对输入样本进行逐变量判断
            featIndex = features.index(key1)
            # 这里每一个key值对应的是根节点特征的不同取值
            for key2 in secondDict.keys():
                if x[featIndex] == key2:
                    # 该分支产生了一个内部节点，则在决策树中继续递归寻找
                    if isinstance(secondDict[key2], dict):
